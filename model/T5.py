...
def evaluate():
    ...
    evaluate_t5(train, test, tokenizer, t5_model)


def evaluate_t5(train, test, tokenizer, t5_model):
    # Create an empty DataFrame to store results
    results_df = pd.DataFrame(columns=['Document1', 'Document2', 'ActualScore', 'PredictedScore'])

    # Iterate through each pair in the test set and calculate similarity scores using T5
    for idx, pair in enumerate(test):
        document1 = pair["doc1"]["text"]
        document2 = pair["doc2"]["text"]
        actual_score = pair["score"]

        # Tokenize and get embeddings for documents using T5
        inputs1 = tokenizer("semantic similarity: " + document1, return_tensors='pt', max_length=512, truncation=True)
        inputs2 = tokenizer("semantic similarity: " + document2, return_tensors='pt', max_length=512, truncation=True)

        with torch.no_grad():
            # Ensure that `decoder_input_ids` is specified
            inputs1["decoder_input_ids"] = inputs1["input_ids"].clone()
            inputs2["decoder_input_ids"] = inputs2["input_ids"].clone()

            output1 = t5_model(**inputs1)
            output2 = t5_model(**inputs2)

            # Use 'encoder_last_hidden_state' instead of 'last_hidden_state'
            embeddings1 = output1['encoder_last_hidden_state'].mean(dim=1)
            embeddings2 = output2['encoder_last_hidden_state'].mean(dim=1)
    ...
    results_csv_path = "../dataset/results_t5.csv"
