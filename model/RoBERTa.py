...
def evaluate():
    ...
    evaluate_roberta(train, test, tokenizer, roberta_model)


def evaluate_roberta(train, test, tokenizer, roberta_model):
    # Create an empty DataFrame to store results
    results_df = pd.DataFrame(columns=['Document1', 'Document2', 'ActualScore', 'PredictedScore'])

    # Iterate through each pair in the test set and calculate similarity scores using RoBERTa
    for idx, pair in enumerate(test):
        document1 = pair["doc1"]["text"]
        document2 = pair["doc2"]["text"]
        actual_score = pair["score"]

        # Tokenize and get embeddings for documents using RoBERTa
        ...
        with torch.no_grad():
            embeddings1 = roberta_model(**inputs1).last_hidden_state.mean(dim=1)
            embeddings2 = roberta_model(**inputs2).last_hidden_state.mean(dim=1)
    ...
    results_csv_path = "../dataset/results_roberta.csv"
