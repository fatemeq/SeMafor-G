...
def evaluate():
    ...
    evaluate_bart(train, test, tokenizer, bart_model)

def evaluate_bart(train, test, tokenizer, bart_model):
    # Create an empty DataFrame to store results
    results_df = pd.DataFrame(columns=['Document1', 'Document2', 'ActualScore', 'PredictedScore'])

    # Iterate through each pair in the test set and calculate similarity scores using BART
    for idx, pair in enumerate(test):
        document1 = pair["doc1"]["text"]
        document2 = pair["doc2"]["text"]
        actual_score = pair["score"]

        # Tokenize and get embeddings for documents using BART
        ...
        with torch.no_grad():
            output1 = bart_model(**inputs1)
            output2 = bart_model(**inputs2)
    ...
    results_csv_path = "../dataset/results_bart.csv"
